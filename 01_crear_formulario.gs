...
function crearFormularioFlora() {
  ...
  // ── Foto ──
  // La subida de archivos vía API solo existe en cuentas Google Workspace.
  // En cuentas de Gmail personal addFileUploadItem no existe → se pide el LINK de Drive.
  var conSubidaArchivo = true;
  if (typeof form.addFileUploadItem === 'function') {
    ...
    conSubidaArchivo = false;
    form.addParagraphTextItem().setTitle('Foto del ejemplar (link de Drive)')
      .setHelpText('Subí la foto a tu Google Drive → botón compartir → "Cualquiera con el enlace" → copiá el link y pegalo acá (opcional).')
      .setRequired(false);
  }
  ...
  Logger.log('════════════════════════════════════════════');
  if (!conSubidaArchivo) {
    Logger.log('ℹ️  CUENTA PERSONAL: la foto se pide como LINK de Drive (la subida directa de');
    Logger.log('    archivos en formularios exige cuenta Workspace). Si preferís subida directa:');
    Logger.log('    abrí el formulario → agregá pregunta "Subir archivo" llamada "Foto del ejemplar"');
    Logger.log('    → Google va a pedir activar "Recopilar direcciones email" (aceptá; ya estaba).');
  }
}
